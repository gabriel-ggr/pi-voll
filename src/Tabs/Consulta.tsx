...
import { converterStringParaData, formatarData } from '../utils/conversoes'
    ...
    async function carregarConsultas() {
      ...
      try {
        ...
        const proximas = consultas.filter((consulta) => {
          if (!consulta?.data) {
            console.warn('Consulta sem data:', consulta);
            return false;
          }
          return new Date(consulta.data) > agora;
        });
  
        const passadas = consultas.filter((consulta) => {
          if (!consulta?.data) {
            console.warn('Consulta sem data:', consulta);
            return false;
          }
    ...
    <ScrollView p="5">
      ...
      {consultasProximas?.map((consulta) => (
        <CardConsulta 
          ...
          data={consulta?.data ? formatarData(converterStringParaData(consulta.data)) : 'Data inválida'}
          foiAgendado
          key={consulta?.id}
